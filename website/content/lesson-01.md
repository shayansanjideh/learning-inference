...
<section class="benchmark-explorer" data-sweep="output" aria-label="Output tokens benchmark explorer"><h3>Explore output length</h3><p>Prompt held at 128 tokens. Select a measured length to compare its latency and throughput with the shortest condition.</p><label for="output-length">Output tokens</label><input id="output-length" type="range" min="0" max="3" value="0" step="1"><div class="length-options" role="group" aria-label="Choose output tokens"></div><div class="bench-readout" aria-live="polite"></div><label for="output-metric">Graph</label><select id="output-metric"><option value="latency_s">Generation time (seconds)</option><option value="output_tokens_per_s">Throughput (output tokens / second)</option></select><div class="bench-plot"></div><p class="bench-comparison" aria-live="polite"></p><p>Each dot is one measured run. The vertical line marks the median. The axis stays fixed when you change length; switching metrics changes its units and scale.</p><details><summary>See the five selected measurements</summary><div class="bench-table"></div></details><noscript><figure class="chart"><picture><source media="(max-width: 600px)" srcset="lesson-01-assets/output-scaling-mobile.png"><img src="lesson-01-assets/output-scaling.png" alt="At 128 prompt tokens, median latency rises from 1.40 to 7.53 seconds as output grows from 8 to 64 tokens; throughput rises from 5.71 to 8.50 tokens per second." width="2160" height="900"></picture><figcaption><b>FIG. 03.</b> Latency and throughput from the same output-length sweep. Points show medians of five runs; whiskers show the full observed range. The two panels use separate labeled scales.</figcaption></figure></noscript></section>
...
<section class="benchmark-explorer" data-sweep="input" aria-label="Prompt tokens benchmark explorer"><h3>Explore input length</h3><p>Output held at 32 tokens. Select a measured length to compare its latency and throughput with the shortest condition.</p><label for="input-length">Prompt tokens</label><input id="input-length" type="range" min="0" max="2" value="0" step="1"><div class="length-options" role="group" aria-label="Choose prompt tokens"></div><div class="bench-readout" aria-live="polite"></div><label for="input-metric">Graph</label><select id="input-metric"><option value="latency_s">Generation time (seconds)</option><option value="output_tokens_per_s">Throughput (output tokens / second)</option></select><div class="bench-plot"></div><p class="bench-comparison" aria-live="polite"></p><p>Each dot is one measured run. The vertical line marks the median. The axis stays fixed when you change length; switching metrics changes its units and scale.</p><details><summary>See the five selected measurements</summary><div class="bench-table"></div></details><noscript><figure class="chart"><picture><source media="(max-width: 600px)" srcset="lesson-01-assets/input-scaling-mobile.png"><img src="lesson-01-assets/input-scaling.png" alt="Five observed generation times for each prompt length. Medians are 3.43 seconds at 64 tokens, 4.06 at 128, and 4.75 at 256; the longest observed run is 10.16 seconds." width="1800" height="960"></picture><figcaption><b>FIG. 04.</b> Each small point is one measured run. Diamonds mark medians. All requests generated 32 tokens. The slow 256-token observation is retained in the chart and raw data.</figcaption></figure></noscript></section>
